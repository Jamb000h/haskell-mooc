...
buildList' start 0 end cur = cur
buildList' start count end cur = buildList' start (count - 1) end (start:cur)
...
sums' cur i
  | i == 0 = cur
  | otherwise = sums' (summer i:cur) (i - 1)
...
maybeMap f [] = [] 
maybeMap f (x:xs) = case f x of Just b -> b : maybeMap f xs
