...
inputInOutput input output = forAll (elements input) (\x -> elem x (map fst output))
...
outputInInput input output = forAll (elements output) (\(x, y) -> (length $ filter (\z -> z == x) input) == y)
...
instance Arbitrary Arg where
  arbitrary = oneof [generateNumber, generateVariable]
    where generateNumber = do
            n <- elements [0..10]
            return (Number n)
          generateVariable = do
