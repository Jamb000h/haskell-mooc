...
totalPrice (MkShoppingEntry name price amt) = price * fromIntegral amt
...
buyOneMore (MkShoppingEntry name price amt) = MkShoppingEntry name price (amt + 1)
...
rgb Green = [0,1,0]
rgb Blue  = [0,0,1]
rgb (Mix c1 c2) = map (/2) $ zipWith (+) (rgb c1) (rgb c2)
...
toNat x 
  | x < 0 = Nothing
  | x == 0 = Just Zero
  | otherwise = Just (PlusOne (maybeNatToNat (toNat (x - 1))))

maybeNatToNat :: Maybe Nat -> Nat
maybeNatToNat (Just x) = x
maybeNatToNat Nothing = Zero
