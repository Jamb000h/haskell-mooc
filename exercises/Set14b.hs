...
deposit db u amt = execute db depositQuery (u, amt)
...
balance db u = do
  res <- query db balanceQuery [u] :: IO [[Int]]
...
parseCommand [c, u]
  | c == (T.pack "balance") = Just (Balance u)
  | otherwise = Nothing
parseCommand [c, u, amt]
  | c == (T.pack "deposit") = parseInt amt >>= \x -> Just (Deposit u x)
  | c == (T.pack "withdraw") = parseInt amt >>= \x -> Just (Withdraw u x)
  | otherwise = Nothing
...
perform db (Just (Balance u)) = do
  res <- balance db u
  return (T.pack (show res))
perform db (Just (Deposit u amt)) = do
  res <- deposit db u amt
  return (T.pack "OK")
perform db (Just (Withdraw u amt)) = do
  res <- deposit db u (-amt)
  return (T.pack "OK")
...
server db request respond = do
  res <- perform db (parseCommand (pathInfo request))
