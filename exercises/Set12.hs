...
count a xs = foldr (\x y -> if x == a then (y + 1) else (y + 0)) 0 xs
...
inBoth xs ys = foldr (\x y -> if elem x ys && notElem x y then  [x] ++ y else y) [] xs
...
instance Foldable List where
  foldr f prev Empty = prev
  foldr f prev (LNode a b) = f a (foldr f prev b)
...
instance Foldable TwoList where
  foldr _ prev TwoEmpty = prev
  foldr f prev (TwoNode a b c) = f a (f b (foldr f prev c))
...
-- Idis on se, että mäpätän vasta kun on saatu tulos Funilta
...
sumTree _ = mempty
-- Monoidit yhdistetään semigroupin associative operatorilla
-- "nollatapaus" saadaan monoidiksi korotetulta semigroupilta jolla on siten mempty
