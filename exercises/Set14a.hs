...
greetText s
  | T.length s > 15 = greetText' $ (T.take 15 s) <> (T.pack "...")
  | otherwise = greetText' s
  where greetText' ss = (T.pack "Hello, ") <> ss <> (T.pack "!")
...
shout :: T.Text -> T.Text
shout s = T.intercalate (T.pack " ") $ map (\(x, i) -> if even i then (T.toUpper x) else x) $ zip (T.words s) [0..]
...
longestRepeat s = longestRepeat' (T.unpack s) 0

longestRepeat' :: String -> Int -> Int
longestRepeat' [] max = max
longestRepeat' xs max = let prefixValue = length $ takeWhile (\x -> x == head xs) xs
                            suffix = dropWhile (\x -> x == head xs) xs
                        in if prefixValue > max
                           then longestRepeat' suffix prefixValue
                           else longestRepeat' suffix max
...
takeStrict n s = TL.toStrict $ TL.take n s
...
byteRange b
  | length (B.unpack b) == 0 = 0
  | otherwise = maximum (B.unpack b) - minimum (B.unpack b)
...
countUtf8Chars b = let decoded = decodeUtf8' b
                   in case decoded of
                     Left e -> Nothing
...
pingpong b = let bInf = BL.fromStrict b
             in BL.cycle (bInf <> BL.reverse bInf)
