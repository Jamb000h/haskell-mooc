...
distinct xs = distinct' (sort xs)

distinct' :: Ord a => [a] -> Bool
distinct' [] = True
distinct' [x] = True
distinct' (x:y:xs) = x < y && distinct' (y:xs)
...
rangeOf (x:y:xs) = let sortedA = sort (x:y:xs)
                       first = head sortedA
                       last = sortedA !! (length sortedA - 1)
                in abs (first - last)
rangeOf _ = 0
...
longest :: Ord a => [[a]] -> [a]
longest xs = let sortedListOfLists = sortByLength xs
                 filtered = filter (\x -> length x == length (sortedListOfLists !! (length sortedListOfLists - 1)))  sortedListOfLists
                 smallestValue = minimum filtered
              in smallestValue
...
incrementKey k = map (\(kx, vx) -> if kx == k then (kx, vx + 1) else (kx, vx))
...
freqs' :: (Ord a) => [a] -> Map.Map a Int -> Map.Map a Int
freqs' [] m = m
freqs' (x:xs) m = let c = Map.findWithDefault 0 x m
                  in freqs' xs (Map.insert x (c + 1) m)
...
transfer from to amount bank
    ...
    | otherwise = case Map.lookup from bank of
                    ...
                    Just sum -> if sum < amount
                                then bank
                                else Map.adjust (+ amount) to (Map.adjust (\x -> x - amount) from bank)
...
maxIndex arr = let kvA = assocs arr
               in maxIndex' (fst (head kvA)) (snd (head kvA)) kvA

maxIndex' :: Ord a => i -> a -> [(i, a)] -> i
maxIndex' mI _ [] = mI
maxIndex' mI max ((i, e):xs) =
    if e > max
    then maxIndex' i e xs
    else maxIndex' mI max xs