...
allValues c (Node x l r) = c x && allValues c l && allValues c r
...
cull val (Node x l r) =
  if x == val
    then Empty
    else Node x (cull val l) (cull val r)
...
search s (Node x l r) = search' [] s (Node x l r)

search' :: Eq a => [Step] -> a -> Tree a -> Maybe [Step]
search' _ _ Empty = Nothing
search' steps s (Node x l r) =
  if x == s
    then Just (reverse steps)
    else let lsteps = search' (StepL : steps) s l
             rsteps = search' (StepR : steps) s r
         in if lsteps == Nothing
              then rsteps
              else lsteps